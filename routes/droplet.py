...
def request_new_instance():
	...
	try:
		...
		nginx_config = generate_nginx_config(instance, droplet, ip, g.user)
		...
		log("ERROR", f"Error creating container for user {g.user.username}: {str(e)}")
		# Cleanup on failure
		try:
...
def generate_guac_token(droplet: Droplet, user: User) -> str:
	...
	return encrypt_token(guac_token, FIXED_AUTH_TOKEN.encode())
...
def droplet(instance_id: str):
	...
	if droplet.droplet_type in ["vnc", "rdp", "ssh"]:
		using_guac = True
		guac_token = generate_guac_token(droplet, g.user)
